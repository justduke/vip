                        <div class="inbox-left-sd">
                        
                            <div class="compose-ml">
                             
                            <a class="btn active" >My Menu</a>
                            
                           
                          
                            </div>
                            
                        <div class="inbox-status">
                            <ul class="inbox-st-nav inbox-ft">
                                <li><a href="index.php?pay_upcoming"><i class="notika-icon notika-sent"></i>Upcoming payment </a></li>
                                <li><a href="index.php?pay_review"><i class="notika-icon notika-refresh"></i> Under Review</a></li>
                                <li><a href="index.php?pay_cleared"><i class="notika-icon notika-menus"></i>Cleared Payment</a></li>
                                
                            </ul>
                        </div>
                        <hr>
                        <div class="inbox-status">
                            <ul class="inbox-st-nav">
                                <li><a href="index.php?myprofile"><i class="notika-icon notika-support"></i> Profile</a></li>
                                <li><a href="index.php?mailbox"><i class="notika-icon notika-mail"></i> Mailbox</a></li>
                                <li><a href="index.php?pay_upcoming"><i class="notika-icon notika-credit-card"></i> Finance</a></li>
                                <li><a href="#"><i class="notika-icon notika-flag"></i> Updates</a></li>
                            </ul>
                        </div>
                        <hr>
                       
                    </div>